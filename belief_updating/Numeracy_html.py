...
def content_block(player, components, **api_kwargs):
    yield t"""<p>Please answer the following four questions. You earn &euro;{C.BONUS_PER_CORRECT} for each correct answer, up to &euro;{C.NUM_SCORE_MAX * C.BONUS_PER_CORRECT} in total. You may use the pen and paper provided.</p>
    ...
    yield t"""<p><i>Question 2.</i> Out of {C.TOWN_POPULATION:,} people in a small town, {C.CHOIR_MEMBERS:,} are members of a choir. Out of these {C.CHOIR_MEMBERS:,} members in the choir {C.CHOIR_MEN} are men. Out of the {C.CHOIR_MEMBERS:,} inhabitants that are not in the choir {C.NON_CHOIR_MEN} are men. What is the probability that a randomly drawn man is a member of the choir?</p>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;<input type="number" name="num_q2" min="0" max="{C.Q2_MAX}" required> %</p>
    """
    for error in components.form['num_q2'].errors:
        yield f'<div class="form-control-errors">{error}</div>'
    yield t"""<p><i>Question {C.Q3_LABEL}.</i> Imagine a six-sided die is thrown. The probability that the die shows a {C.DIE_FACE_6} is twice as high as the probability of each of the other numbers. On average, out of these {C.Q3_MAX} throws, how many times would the die show the number {C.DIE_FACE_6}?</p>
